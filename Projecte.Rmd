...
a further research with the sample. It will be taxonomically classified using
[GTDB](https://gtdb.ecogenomic.org/). As an output a taxonomical tree
will promote the identification of the bacterial sepecies present in our
sample.
...
```{r Figure 1, echo= FALSE, fig.align = 'center', out.width = "50%", out.extra='trim={0 0.4cm 0 0.4cm},clip', fig.cap = "\\scriptsize \\textbf{Figure 1.}. Quality check reads scores"}
...
```{r Figure 5, echo= FALSE, fig.align = 'center', out.width = "90%", out.extra='trim={0 0.5cm 0 0.5cm},clip', fig.cap = "\\scriptsize \\textbf{Figure 5.} Bacterial Taxonomy classification"}
...
```{r Figure 7, echo= FALSE, fig.align = 'center', out.width = "75%", out.extra='trim={0 0.2cm 0 0.2cm},clip', fig.cap = "\\scriptsize \\textbf{Figure 7.} ANI distance estimation"}
knitr::include_graphics(here::here("ANI_estimate_ambos_pr5.png"))
```

```{r Figure 8, echo= FALSE, fig.align = 'center', out.width = "35%", out.extra='trim={0 0.2cm 0 0.2cm},clip', fig.cap = "\\scriptsize \\textbf{Figure 8.} fastANI representation"}
...
```{r Figure 9, echo= FALSE, fig.align = 'center', out.width = "62%", out.extra='trim={0 0.28cm 0 0.28cm},clip', fig.cap = "\\scriptsize \\textbf{Figure 9.} Functional categories encoded in MAG (left) and reference (right) genomes"}
...
```{r Figure 10,  echo=FALSE, fig.align = 'center', out.width = "35%", fig.show='hold',fig.cap = "\\scriptsize \\textbf{Figure 10.} MAG (left) and reference (right) genes graphic"}
library("ape")
library("ggplot2")
...
The gene selected is categorized within the "Clustering-based subsystems" gene group, known for its involvement in Bacterial Cell Division. This gene group encompasses a total of 5 coding genes in both our MAG and the reference genome. More precisely, the gene IGLBDLCO_01642 found in the MAG and PKGELIAN_01503 identified in the reference genome share a sequence length of 963 base pairs (bp). This sequence encodes a protein comprising 320 aminoacids,known as the Cell division protein FtsQ.

The multiple nucleotide sequence alignment between the MAG and the
reference genome is depicted in *Supplementary Info. 1*, revealing 9 mismatches across the 963 bp sequence length. These mismatches are visually
highlighted in *Figure 11*, displaying a gap in the consensus sequence
(black box).

```{r Figure 11, echo= FALSE, fig.align = 'center', out.width = "90%", out.extra='trim={0 0.2cm 0 0.2cm},clip', fig.cap = "\\scriptsize \\textbf{Figure 11.} Missmatches between FstQ reference (up) and MAG (down) nucleotide sequence."}
...
*Supplementary info 2* and more explicitly in *Figure 12*. Among the 9 previous mismatches, it's evident that only 2 involve a non-synonymous mutation in our MAG when compared to the reference:
...
# CONCLUSIONS
Having analyzed our sample throughout all 5 practicals, we can set a series of conclusions from it.

First, in practical 4 our sequence was first computed and examined in order to elucidate its quality and possible compilation to an integrated genome. Its quality was believed to be quite high, as many of the reads presented such trait. Therefore, trimming was unnecessary and could be directly assembled. Several contigs were created which enabled the composition of 7 bins. Among those, only the third fulfilled both completeness and contamination requirements.

Then, using the chosen bin, and comparing it to a reference genome we could identify the sample procedence. The whole genome assembly was identified as *Bifidobacterium dentium*. *Bifidobacterium* is a genus of gram-positive, non motile, anaerobic bacteria. They are mainly known as inhabitants of the gastrointestinal tract in mammals, although some strains have been isolated in both vaginal and oral microbiota. Their establishment in the human intestine is believed to be linked to beneficial health effects, such as immune development, neuromodulation, and microbiotal homeostasis. In particular, *Bifidobacterium dentium* is found in both intestinal and oral environments, and has been related to goblet cell maturation, serotonin production, and GABA production, regulating the gut-brain-axis. 

Afterwards, the genome was further examined by studying its functional categories. It presented a wide variety of physiological functions, which were coded in several identified genes. 


# REFERENCES

1. GTDB - Genome Taxonomy Database [Internet]. [cited 2023 Nov]. Available from: https://gtdb.ecogenomic.org/
2. Letunic I, Bork P. Interactive Tree Of Life (iTOL) v5: an online tool for phylogenetic tree display and annotation. Nucleic Acids Research. 2021 Jul 2;49(W1):W293–6. 
3. Waterhouse AM, Procter JB, Martin DMA, Clamp M, Barton GJ. Jalview Version 2—a multiple sequence alignment editor and analysis workbench. Bioinformatics. 2009 May 1;25(9):1189–91. 
4. Katoh K, Rozewicki J, Yamada KD. MAFFT online service: multiple sequence alignment, interactive sequence choice and visualization. Briefings in Bioinformatics. 2019 Jul 19;20(4):1160–6. 
5. Point accepted mutation. In: Wikipedia [Internet]. 2023 [cited 2023 Nov]. Available from: https://en.wikipedia.org/w/index.php?title=Point_accepted_mutation&oldid=1184343745
6. KBase [Internet]. [cited 2023 Nov]. Welcome to KBase Predictive Biology. Available from: https://www.kbase.us/
